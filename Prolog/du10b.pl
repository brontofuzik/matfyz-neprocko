...
%%%%% generuj_zac/4(+M,+L,+R,-Z) :- Vygeneruje zaciatocny stav pre M misionarov, L ludozrutov a R riek, vysledok vrati ako zoznam Z.
...
%%%%% generuj_kon/4(+M,+L,+R,-K) :- Vygeneruje koncovy stav pre M misionarov, L ludozrutov a R riek, vysledok vrati ako zoznam K.
...
%%%%% prevoz/2(S1,S2) :- Prevoz medzi stavom S1 a S2.
...
%%%%% bezpecny/1(+B) :- Vyhodnoti bezpecnost na brehu B.
bezpecny(b(M,L,_)) :-
	M==0;
	(
		M=\=0,
...
%%%%% hrana/2(+S1,+S2) :- Zo stavu S1 je mozne prejst do (bezpecneho) stavu S2.
%%%%% Interface pre grafovy algoritmus (BFS) nejcesta/3.
hrana(S1,S2) :-
	prevoz(S1,S2).

%%%%% nejcesta/3(Start,Cil,Cesta) :- najde nejkratsi cestu z vrcholu Start do vrcholu Cil
...
%%%%% vypis/1(+Z) :- Vypise prvky zoznamu Z (najkratsie cesty), kazdy do samostatneho riadku. 
vypis([]) :-
	!.
vypis([H|T]) :-
	write(H),
...
%%%%% uloha/4(+M,+L,+R,-C1) :- Riesi ulohu, vzsledok vracia ako zoznam vsetkych najkratsich ciest (postupnosti stavov) C1.	
uloha(M,L,R,C1) :-
	generuj_zac(M,L,R,Z),
	generuj_kon(M,L,R,K),
	nejcesta(Z,K,C1),
